...
public class ItchService : Abstractions.IItchService
{
    ...
    public string GetLoginUrl()
    {
        return QueryHelpers.AddQueryString(
            configuration["Itch:Authority"] ?? throw new InvalidOperationException("Incomplete Itch configuration: missing Authority field"),
            new Dictionary<string, string?> {
                {
                    "client_id",
                    configuration["Itch:ClientId"] ?? throw new InvalidOperationException("Incomplete Itch configuration: missing ClientId field") },
                {
                    "scope",
                    "profile:me"
                },
    ...
    public async Task<ItchProfile> FetchProfile(string accessToken)
    {
        ...
        return await response.Content.ReadFromJsonAsync<ItchProfile>(GetJsonSerializerOptions()) ?? throw new InvalidOperationException("Itch profile response was null");
    }
    ...
    public async Task<ItchCredentialsInfo> CheckCredentials(string accessToken)
    {
        ...
        return await response.Content.ReadFromJsonAsync<ItchCredentialsInfo>(GetJsonSerializerOptions()) ?? throw new InvalidOperationException("Itch profile credentials info was null");
    }
    ...
    /// </summary>
    private async Task<HttpResponseMessage> MakeRequest(string path, string apiToken, HttpMethod method)
    {
